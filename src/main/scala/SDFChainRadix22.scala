...
class SDFChainRadix22[T <: Data: Real: BinaryRepresentation](val params: FFTParams[T]) extends Module with HasIO {
  ...
  val outputWires =
    if (params.trimEnable) Wire(Vec(numStages, params.protoIQstages.last.cloneType))
    else Wire(Vec(numStages, io.out.bits.cloneType))
  ...
  sdf_stages.map(_.io).zipWithIndex.foldLeft(input_data) {
    case (stg_in, (stg_io, index)) => {
      ...
      val isMulTw =
        if (params.decimType == DIFDecimType) ((index + 1) % 2 == 0 && !doNothing)
        else (((index + 1) % 2 == 0) && (numStages % 2 == 1)) || (((index + 1) % 2 == 1) && (numStages % 2 == 0))
      ...
      if (isMulTw && !doNothing) {
        ...
        val uniqueTwiddleTable = VecInit(
          uniqueTwiddleIdxs.map(t =>
            DspContext.withTrimType(Convergent) {
              DspComplex.wire(
                real = ConvertableTo[T]
                  .fromDoubleWithFixedWidth(math.cos(2 * math.Pi * t / stageN), params.protoTwiddle.real),
                imag = ConvertableTo[T]
                  .fromDoubleWithFixedWidth(-math.sin(2 * math.Pi * t / stageN), params.protoTwiddle.imag)
              )
            }
          )
        )
        ...
        if (params.decimType == DIFDecimType) {
          ...
          val addressRetime =
            if (params.runTime)
              RegEnable(
                cntr_wires(currIndex) + 1.U - (cumulativeDelaysWires(index + 1) - cumulativeDelayWire),
                enableVector(currIndex)
              )
            else cntr_wires(currIndex) - (cumulativeDelaysWires(index + 1) - cumulativeDelayWire)
...
//           DspContext.alter(DspContext.current.copy(trimType = NoTrim, overflowType = Grow, complexUse4Muls = params.use4Muls)) { stg_in context_* twiddles(index) }
          ...
          val mulres =
            if (params.decimType == DIFDecimType)
              ...
              ) { stg_io.out.context_*(twiddles(index)) }
            else
              DspContext.alter(
                DspContext.current.copy(
              ...
              ) { stg_in.context_*(twiddles(index)) }
        ...
        if (!doNothing) {
          ...
          val invertSignal =
            if (params.decimType == DIFDecimType)
              ShiftRegister(
                Mux(
                  stg_io.cntr < (delays(index).U),
                  Mux(stg_io.cntr < ((delays(index).U) >> 1), false.B, true.B),
                  false.B
                ),
                params.numAddPipes,
                true.B
              ...
              Mux(
                stg_io.cntr < (delays(index).U),
                false.B,
                Mux(stg_io.cntr < ((delays(index) * 3 / 2).U), false.B, true.B)
              )
  ...
  if (latency == 0) {
    when(fftOrifft === true.B) {
      ...
      if (params.trimEnable && div2Num > 0) {
        fft_out_bits.real := DspContext.alter(DspContext.current.copy(trimType = Convergent, binaryPointGrowth = 0)) {
          output.real.div2(div2Num)
        }
        fft_out_bits.imag := DspContext.alter(DspContext.current.copy(trimType = Convergent, binaryPointGrowth = 0)) {
          output.imag.div2(div2Num)
        }
      } else {
        fft_out_bits := output
      }
    ...
    when(fftOrifft === true.B) {
      if (params.trimEnable && div2Num > 0) {
        ...
        fft_out_bits.real := DspContext.alter(DspContext.current.copy(trimType = Convergent, binaryPointGrowth = 0)) {
          outQueue.io.deq.bits.real.div2(div2Num)
        }
        fft_out_bits.imag := DspContext.alter(DspContext.current.copy(trimType = Convergent, binaryPointGrowth = 0)) {
          outQueue.io.deq.bits.imag.div2(div2Num)
        }
      } else {
...
class SDFStageRadix22[T <: Data: Real: Ring: BinaryRepresentation](
  ...
  if (useGrow) {
    ...
    val check_overflow = params.protoIQ.real match {
      case fp: FixedPoint =>
        Seq(butt_outputs(0).real, butt_outputs(0).imag, butt_outputs(1).real, butt_outputs(1).imag).map(sGrow => {
