...
object TwiddleRadix22LookUp {
  def apply[T <: Data: Real](address: UInt, stageN: Int, protoTwiddle: DspComplex[T]): DspComplex[T] = {
    ...
    val uniqueTwiddleTable = VecInit(
...
class SDFChainRadix22RunTime[T <: Data: Real: BinaryRepresentation](val params: FFTParams[T])
    extends Module
    with HasIO {
  ...
  val delayLog2s =
    if (params.decimType == DITDecimType) (0 until numStages)
    else (0 until numStages).reverse // log2(delay) of each nth stage
  ...
  val muxOutputTw =
    Wire(MixedVec((0 to numStages - 1).map { i =>
      params.protoIQstages(i)
    })) //Wire(Vec(numStages, params.protoIQstages(numStages - 1)))

  // try to reduce this condition
  val isShiftAddr =
    !((((numStages % 2) == 0).B && (regNumStages % 2.U === 0.U)) || (((numStages % 2) == 1).B && (regNumStages % 2.U === 1.U)))
  ...
  activeStages.zip(oddEvenStages).zipWithIndex.map {
    case ((active, oddEven), index) => {
  ...
  (0 until numStages by 2).zipWithIndex.map {
    case (logStage, index) => {
  ...
  val lastOut =
    if (params.numAddPipes == 0 && params.numMulPipes == 0) pktEnd && (state === sFlush)
    else Mux(lastWait, lastIndeed && pktEnd, pktEnd && state === sFlush)
    ...
    case ((delayLog2, delay), ind) => {
      ...
      when(oddEvenStages(ind) && !doNothing && activeStages(ind)) {
        ...
        if (params.decimType == DIFDecimType) {
          ...
          twiddlesVector(ind) := ShiftRegister(twiddles(indexToTwiddles), params.numAddPipes, true.B)
        } else {
        ...
        .elsewhen(!doNothing && activeStages(ind)) {
          if (params.decimType == DIFDecimType)
            invertSignals(ind) := ShiftRegister(
              Mux(stage.io.cntr < (delay.U), Mux(stage.io.cntr < ((delay.U) >> 1), false.B, true.B), false.B),
              params.numAddPipes,
              true.B
            )
          else
            invertSignals(ind) := Mux(
              stage.io.cntr < (delay.U),
              false.B,
              Mux(stage.io.cntr < ((delay * 3 / 2).U), false.B, true.B)
            )
          twiddleAddr(ind) := 0.U
  ...
  (0 until numStages).foldLeft(((enableInit, cnt))) {
    case ((stageEn, stageCnt), currStage) => {
      ...
      (
        ShiftRegisterWithReset(stageEn, outputLatency, resetData = false.B, reset = state_next === sIdle, true.B),
        ShiftRegisterWithReset(stageCnt, outputLatency, resetData = 0.U, reset = state_next === sIdle, true.B)
      )
    }
  }

  sdf_stages.zipWithIndex.map {
    case (e, idx) => {
      val condition =
        if (params.decimType == DIFDecimType) (idx.U < (numStages.U - (regNumStages))) else (idx.U >= regNumStages)
  ...
  sdf_stages.map(_.io).zipWithIndex.foldLeft(input_data) {
    case (stg_in, (stg_io, index)) => {
      ...
      val isMulStage =
        ((params.decimType == DIFDecimType) && (index % 2 == 1)) || (params.decimType == DITDecimType && ((numStages % 2 == 0 && index % 2 == 0) || (numStages % 2 == 1 && index % 2 == 1)))
      ...
      if (
        (index == 0 && params.decimType == DIFDecimType) || (index == (numStages - 1) && params.decimType == DITDecimType)
      ) {
        ...
        tmpInput := ShiftRegister(muxOutputTw(index), complexMulLatency, true.B)
      } else {
        if (
          (index == (numStages - 1) && params.decimType == DIFDecimType) || ((index == 0) && params.decimType == DITDecimType)
        ) {
          if (params.decimType == DIFDecimType) {
            tmpInput := ShiftRegister(stg_io.out, complexMulLatency, true.B)
            muxOutputTw(index) := stg_io.out
          } else {
            tmpInput := ShiftRegister(input, complexMulLatency, true.B)
          ...
          if (params.decimType == DIFDecimType) {
            ...
            inputToMul := Mux(
              activeStages(index) && oddEvenStages(index),
              stg_io.out,
              Mux(activeStages(index), outNextStage, rstProtoStg)
            )
            twiddle := Mux(
              activeStages(index) && oddEvenStages(index),
              twiddlesVector(index),
              Mux(activeStages(index), twiddlesVector(index + 1), rstProtoTwiddle)
            )
          } else {
            ...
            inputToMul := Mux(
              activeStages(index) && oddEvenStages(index),
              stg_in.asTypeOf(stg_io.in),
              Mux(activeStages(index), outputVal, rstProtoStg)
            )
            val twiddleVal = if (numStages % 2 == 0) twiddlesVector(index - 1) else twiddlesVector(index + 1)
            twiddle := Mux(
              activeStages(index) && oddEvenStages(index),
              twiddlesVector(index),
              Mux(activeStages(index), twiddleVal, rstProtoTwiddle)
            )
          }
          DspContext.alter(
            ...
            val mulres = DspContext.alter(
              DspContext.current
                .copy(trimType = params.trimType, overflowType = Grow, complexUse4Muls = params.use4Muls)
            ) { inputToMul.context_*(twiddle) }
            muxOutputTw(index) := DspContext.withTrimType(params.trimType) { mulres.trimBinary(bpos) }
          }
          tmpInput := Mux(
            ~(activeStages(index) && oddEvenStages(index)),
            ShiftRegister(invertData, complexMulLatency, true.B),
            muxOutputTw(index)
          )
        } else {
...
//  val lastStageEn =  enableVector(regNumStages - 1)
  ...
  val output =
    if (params.decimType == DIFDecimType) ShiftRegister(outputWires.last, complexMulLatency, true.B)
    else outputWires(regNumStages - 1.U)
  ...
  val outQueue = Module(
    new Queue(chiselTypeOf(sdf_stages.last.io.out), entries = latency + 1, pipe = true, flow = true)
  )
  outQueue.io.enq.bits := output
  outQueue.io.enq.valid := outValid
  outQueue.io.deq.ready := io.out.ready
  val scalar =
    if (params.expandLogic.sum == 0 && !params.keepMSBorLSBReg) ConvertableTo[T].fromDouble(1.0)
    else ConvertableTo[T].fromDouble(1.0 / params.numPoints.toDouble)
