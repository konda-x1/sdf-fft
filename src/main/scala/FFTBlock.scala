...
abstract class FFTBlock[T <: Data: Real: BinaryRepresentation, D, U, E, O, B <: Data](
  params:          FFTParams[T],
  beatBytes:       Int,
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    if (params.fftDirReg) {
      ...
      commonFields =
        commonFields :+ RegField(1, fftDir, RegFieldDesc(name = "fftDir", desc = "transform direction: fft or ifft"))
    }
...
class AXI4FFTBlock[T <: Data: Real: BinaryRepresentation](
  params:          FFTParams[T],
  address:         AddressSet,
  _beatBytes:      Int = 4,
...
class TLFFTBlock[T <: Data: Real: BinaryRepresentation](
  ...
  implicit p: Parameters)
    extends FFTBlock[T, TLClientPortParameters, TLManagerPortParameters, TLEdgeOut, TLEdgeIn, TLBundle](
      params,
      beatBytes
    )
    with TLDspBlock
...
object FFTDspBlockTL extends App {
  ...
  val lazyDut = LazyModule(
    new TLFFTBlock(paramsFFT, AddressSet(baseAddress + 0x100, 0xff), beatBytes = 4, configInterface = false)
...
object FFTDspBlockAXI4 extends App {
  ...
  implicit val p: Parameters = Parameters.empty
  val lazyDut = LazyModule(
    new AXI4FFTBlock(paramsFFT, AddressSet(baseAddress + 0x100, 0xff), _beatBytes = 4, configInterface = false)
...
object FFTDspBlockAXI4WithConfig extends App {
  ...
  implicit val p: Parameters = Parameters.empty
  val lazyDut = LazyModule(
    new AXI4FFTBlock(paramsFFT, AddressSet(baseAddress + 0x100, 0xff), _beatBytes = 4, configInterface = true)
      with AXI4FFTStandaloneBlock {
